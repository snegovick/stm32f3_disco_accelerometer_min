...
CFLAGS += -Wall -std=gnu99 -Os -funsigned-char
CFLAGS += -MD -MP -MT $(*F).o -MF $(@F).d 
CFLAGS += -I./ -I./util -I./hal
...
SOURCES := $(wildcard $(STD_PERIPH_LIB)/STM32F30x_StdPeriph_Driver/src/*.c)
SOURCES += $(wildcard ./*.c)
SOURCES += $(wildcard ./hal/*.c)
SOURCES += $(wildcard ./util/*.c)
#SOURCES := $(wildcard $(STD_PERIPH_LIB)/Utilities/*.c)
SOURCES += ./ext_lib/Utilities/stm32f3_discovery_lsm303dlhc.c
